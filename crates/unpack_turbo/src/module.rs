...
use crate::{
    ...
    ident::AssetIdent,
    reference::{EsmAssetReference, ModuleReference, ModuleReferences},
    source::Source,
};
...
impl EcmascriptModuleAsset {
    ...
    pub fn new(source: ResolvedVc<Box<dyn Source>>) -> Vc<EcmascriptModuleAsset> {
        EcmascriptModuleAsset { source }.cell()
    }
    #[turbo_tasks::function]
...
impl EcmascriptModuleAsset {
    // codegen
    pub fn module_content(self: Vc<Self>) -> Vc<EcmascriptModuleContent> {
...
pub async fn analyze_ecmascript_module(
    ...
    for request in requests {
        ...
        let reference: Vc<Box<dyn ModuleReference>> =
            Vc::upcast(EsmAssetReference::new(origin_path, request.to_string()));

        references.push(reference.to_resolved().await?);
    }
    Ok(AnalyzeEcmascriptModuleResult { references }.cell())
}
