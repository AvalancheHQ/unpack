...
use turbo_tasks::{
    FxIndexSet, ReadRef, ResolvedVc, TryFlatJoinIterExt, ValueToString, Vc, value_impl,
};
use turbo_tasks_fs::FileSystemPath;

use crate::{
    chunk::chunk_reference::{ChunkableModuleReference, ChunkingType, ChunkingTypeOption},
    file_source::FileSource,
    module::{EcmascriptModuleAsset, Module, Modules},
    module_graph::ExportUsage,
};
...
pub async fn primary_chunkable_referenced_modules(
    ...
    let modules = module
        ...
        .iter()
        .map(|reference| async {
            if let Some(chunk_module_reference) =
            ...
            {
                let resolved = chunk_module_reference
                    .resolve_reference()
                    .resolve()
                    .await?
                    .primary_modules()
...
impl ValueToString for EsmAssetReference {
    fn to_string(self: turbo_tasks::Vc<Self>) -> Vc<RcStr>
    where
        Self: Sized,
    {
        todo!()
